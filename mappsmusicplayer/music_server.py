...
from mappsmusicplayer.data_manager import DataManager
...
class MusicServer(Logger, EventFiring):
    ...
    It sets up all of the different components and a gRPC music_server
    """
    _config_section = 'musicserver'

    def __init__(self, config: RawConfigParser):
        """
        :param config: The configuration for the music music_server
        :type config: RawConfigParser
        """
        super(MusicServer, self).__init__()
        self._event_callbacks = {}
        # initialize callback lists
        for attribute in [attr for attr in dir(self.Events())
                          if not callable(attr) and not attr.startswith("__")]:
            self._event_callbacks[attribute] = []
        self._config = config
        self.__process_conf__()
        # setup music_server components
    ...
    def serve(self):
        """
        This method starts the music music_server and listens for incoming connections
        """
        cmd = ''
        self._gserver.start()
    ...
    def __process_conf__(self):
        ...
        self._connection_count = \
            self._config.get(self._config_section, 'connection_count', fallback=5)
