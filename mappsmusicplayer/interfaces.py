...
class Logger:
    """
    The base class for all classes in music_server package
    """
    ...
    def log(self, content: object):
        if self._logging and LOG:
            b("[*" + str(self.__class__.__name__) + "* | " +
              datetime.now().strftime('%H:%M:%S')
              + "] " + str(content))

    def set_logging(self, state: bool):
        """ Set the logging state of this class
        If true then it's able to log otherwise it is not
...
class EventFiring:
    ...
    def __init__(self):
        ...
        for attr in [attr for attr in vars(self.__class__.Events)
